...
document.addEventListener('DOMContentLoaded', async()=>{setLayout('reports','Báo cáo','Chọn đúng nhu cầu, xem kết quả và xuất dữ liệu khi cần'); await load(); ['reportType','deptFilter','groupFilter','searchInput'].forEach(id=>{q(id).addEventListener('input',applyFilter); q(id).addEventListener('change',applyFilter);}); q('filterBtn').onclick=applyFilter; q('exportBtn').onclick=exportExcel; q('loadKpiBtn').onclick=loadKpi; q('exportKpiBtn').onclick=exportKpiExcel; q('exportDataQualityBtn').onclick=exportDataQualityExcel;});
